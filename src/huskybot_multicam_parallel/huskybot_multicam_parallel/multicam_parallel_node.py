#!/usr/bin/env python3
# filepath: /home/jezzy/huskybot_v11/src/huskybot_multicam_parallel/huskybot_multicam_parallel/multicam_parallel_node.py
...
class MultiCamParallelNode(Node):
    def __init__(self):
        ...
        # ✅ MEGA FIXED: ABSOLUTE CONSTANT canvas size with MAXIMUM FOV and NO SIDE SPACE
        self.CELL_WIDTH = 1500   # MAXIMAL width to eliminate side spaces completely
        self.CELL_HEIGHT = 950   # INCREASED proportionally for better display
    ...
    def create_grid_display(self):
        ...
        try:
            ...
            for config in self.camera_configs:
                ...
                if img is not None:
                    # ✅ MEGA FIXED: Maintain aspect ratio for FULL FOV (rectangular view)
                    # Get original dimensions
                    orig_height, orig_width = img.shape[:2]
                    
                    # Calculate scaling factor to fit within cell while maintaining aspect ratio
                    scale_w = self.CELL_WIDTH / orig_width
                    scale_h = self.CELL_HEIGHT / orig_height
                    ...
                    # Create cell with black background
                    cell_img = np.zeros((self.CELL_HEIGHT, self.CELL_WIDTH, 3), dtype=np.uint8)
                    
                    # Center the resized image in the cell
                    start_y = (self.CELL_HEIGHT - new_height) // 2
                    start_x = (self.CELL_WIDTH - new_width) // 2
                    end_y = start_y + new_height
                    end_x = start_x + new_width
                    
                    cell_img[start_y:end_y, start_x:end_x] = img_resized
                    canvas[img_y_start:img_y_end, x_start:x_end] = cell_img
                else:
